...
flowchart TD
    ...
    B --> C["Key generated + seed backup ceremony<br/>(A1-03, one screen) — recovery-focused copy"]
    C --> GD["Gas deposit (A1-04, decision 87):<br/>fund the new account for the<br/>recovery submission — auto-skipped<br/>when it already holds enough"]
    GD --> E[→ Flow D]
...
Decisions:
- ✅ Fast-track = four steps (87): **warning screen · extension password · key creation with the seed backup ceremony on one screen (A1-03) · gas deposit**. Nothing else.
...
Building blocks:
- **Methods (decision 80):** passkey, guardians (people's wallets, EOA or SCW), **zkPassport**, Anon Aadhaar. zkPassport's mechanics (what the user presents, proof budget, enrollment secret) need a research pass before the old email screens (C-05e/k/n, D-07e) are redrawn. History of the dropped ZK Email method lives in the decisions log only (16/55/77/80).
- **ONE recovery path per account (decision 73):** there is no OR-of-paths level. An account has exactly one path: required method rows and ANY-of groups, all combined with AND. Examples: `passkey AND [2 of 3](G1, G2, zkpassport)` · `[2 of 3](G1, passkey, aadhaar)` · `passkey AND zkpassport` (both required). The UI keeps calling it the **"recovery path"**; the methods stay **"recovery methods"**.
- **ANY-of groups are generic:** a group's members can be ANY method instances — guardians, passkeys, zkPassport, Aadhaar — mixed freely. Guardians are not special: "3 of 5 guardians" is simply a group whose five members happen to be guardians. No guardian-specific threshold concept exists in the UX.
...
flowchart TD
    ...
    E --> F["5 · Waiting period — 48h default,<br/>changeable, minimum 24 hours<br/>(wallet floor — the chain enforces<br/>none; decision 100)"]
    F --> PW["6 · Recovery password — REQUIRED at Private<br/>and Shape visible, ABSENT at Public (90).<br/>Password + Confirm password (decision 95).<br/>It encrypts the recovery config values<br/>(commit-reveal, decision 84) and carries the<br/>visibility choice: Private (default) /<br/>Shape visible / Public<br/>(Public = no password is set)"]
    PW --> DR["7 · Optional DRY RUN (ships V1):<br/>a local rehearsal of the recovery —<br/>complete each method row as if the<br/>key were lost today (decision 47)"]
    DR --> G["8 · Review + save → submit config<br/>(one confirmation, Q28)"]
    G --> H["9 · Recovery Card download<br/>+ enable alerts opt-in"]
    H --> I["10 · Offer: apply to other accounts<br/>(never by default; see R10 constraints)"]
```
...
- ✅ **Alerts opt-in (step 9):** request notification permission here — Flow D2 depends on it. If denied: degraded banner-only behavior, stated to the user.
...
- **Required wherever something is encrypted (decision 90):** the password step is **mandatory** at **Private** and at **Shape visible**, and **absent** at **Public** (nothing is encrypted there, so there is nothing to set — decision 84). The step is not skippable.
...
- **Honest consequence, stated on the step (decision 90 — this line IS setup disclosure #6, "the backup trade", decision 98/REC-35, MVP):** the recovery password is **printed on the Recovery Card**. Anyone holding the card can therefore decrypt and **read** the configuration values — they still cannot recover, because the recovery path itself must be satisfied. But if the card **and** the password are both lost at Private or Shape visible, a fresh device **cannot begin a recovery at all**. Copy must say both halves. Disclosure #6 is **already carried by this line** — tag it, align its wording, and do **not** add a second backup-trade paragraph to the step.
- **Per-level reveal lines on the visibility radio (disclosure #5, decision 98, MVP copy):** each of the three options carries **one line saying what that level lets a stranger see**, rendered under the option label, not in a help sheet. **Private** (default) — "Nothing is READABLE without the password — not even which methods you chose. Anyone can still see that this account HAS a recovery setup." **Shape visible** — "anyone can see the SHAPE of your setup — that it takes a passkey and two of three people — but not which passkey or which people. A fresh device shows the shape without the password." **Public** — "anyone can read the whole setup: every method, every guardian address, every threshold. No recovery password is set." The three lines are the user's only view of the trade, so they must state the reveal, not the reassurance. No level hides the existence of a setup (decision 99) — never write "nobody can see that you have one".
...
### Recovery Card (step 9)

Contents — **only**: the account address, **the recovery password** (decision 90), a short "how to start recovery" guide, and the line "this card alone cannot move funds." The card carries the real password, so no password *hint* exists. The account address on the card is also the answer to fresh-device account discovery (REC-31, Flow D chapter 1).

**Honesty consequences of printing the password (decision 90):** anyone who holds the card can **decrypt and read** the configuration values — which methods, which guardians, which thresholds. They still **cannot recover**: the recovery path itself must be satisfied, and reading the configuration does not satisfy it. The card's storage advice hardens accordingly — at Private and Shape visible it is not a "store it insecurely" artefact. And the residual risk is stated on the card and on C-06e: lose **both** the card and the password at Private or Shape visible, and a fresh device **cannot begin a recovery**.

Explicitly **kept off the card** (decided, with rationale): the method inventory, guardian addresses, thresholds, and waiting-period values. (Guardian *names* do not exist anywhere — decision 43: a guardian is an address, shown as blockie + address or ENS; the wallet stores no contact labels.) Reason: a stolen or copied card must not become a **map of exactly which methods to phish**. The address alone is public information anyway; the configuration is the secret. The card does **not** target insecure storage (decision 90) — it carries the password that unlocks the configuration, so a thief who holds the card can read that map from the encrypted log. Keeping the inventory off the card still helps (it costs the thief a lookup and keeps a *photographed* card less useful), but the card's own copy must ask for real storage care.
...
- ✅ **Cross-account linkability is conditional (decision 92).** Reusing the same passkey or the same guardian across accounts is **invisible at rest ONLY if the SDK mints fresh per-account salts**. Their R-16 attack: under salt reuse, recovering one account **unmasks the other account's setup** — the commitment is recomputable across accounts. So the SDK duty (per-account, per-credential salt minting) is what makes reuse safe, and the UX must not promise privacy the salts do not deliver.
- ✅ **The linkability warning lives at the submission/approval moment (decision 92)**, not at C-09 (setup), where nothing is revealed yet: it sits where the reveal actually happens — the submit confirmation (D-11) and the guardian approval surfaces (E1, the D-07 manual row). Copy: "Submitting this recovery publishes the credentials it uses. Anyone can then see that the same passkey/guardian also protects your other accounts." The "create a fresh passkey for this account" offer stays at setup as the mitigation, and it must not claim that reuse is already public.
- Never applied by default.

Other decisions:
- ✅ All 4 methods in v1. Aadhaar for everyone, listed last, badged.
- ✅ **Our presets take precedence over their tiering (decision 94).** We do **not** adopt the contracts team's primary/secondary method weighting: it is a verdict on rule quality, which is wallet-side by decision 81(a). Preset 2 (`passkey AND zkPassport`) stays. The method set stays passkey + guardians + Anon Aadhaar + zkPassport — reaffirming decisions 79 and 80 — and we keep assuming Anon Aadhaar works (SDK assumption A2), so their Q-7 contingency does not change our count.
- ✅ **A two-method inventory defaults to one [1 of 2] ANY-of group** (decisions 75/37): passkey + ID builds `[1 of 2](passkey, zkPassport)` — one group, never two required rows and never an AND pair. Lockout is the larger risk at that level (Diana's shape). The waiting period stays the flat 48h default.
...
- ✅ Waiting period (decision 74): **one per account** — the path's waiting period. 48h default, changeable, with a **24-hour minimum** in the picker: that floor is the wallet's own (decision 100), and the chain enforces none at all (decision 85), so no copy may say the contract rejects a shorter wait. The dominated-path warning is retired — with a single path, path domination cannot exist.
...
**Config visibility (decision 84, commit-reveal; levels named by decision 99):** the config is not natively public. Values live in **encrypted logs** — the setup event's backup field — and the recovery password decrypts them; the user decides what to expose on the recovery-password step (C-06e). The three levels, described by what the setup-event log encrypts: **Private** (the **default** — the log encrypts the shape **and** the values, so nothing in it is readable without the password) · **Shape visible** (the log carries the shape in the clear — "a passkey and two of three people" — and the values encrypted) · **Public** (the log encrypts nothing, and **no recovery password is set at all**, so there is nothing to enter later). The recoverer enters the recovery password to decrypt whatever was encrypted (e.g. which guardians to contact). **What no level hides is that a setup exists at all** — the commitment sits in storage either way (their I-16) — so copy says "nobody can **read** your setup" and never "nobody can see that you have one" (decision 99). **Shape visible is ASSUMED available (decision 97):** it has no counterpart in the kit's current model, so kit-ask REC-24 carries the confirmation; with Private as the default, a negative answer removes the middle option without touching the default readout.

**D-06e copy (decisions 90 + 56 — "you can still recover, the values just stay hidden" is wrong at Private and at Shape visible):** the recovery password is **printed on the Recovery Card**, so the normal answer to "I forgot it" is "read it off your card". The honest residual is narrower and harder: at Private or Shape visible, if the user has lost **both** the card **and** the password, a fresh device **cannot begin a recovery** — there is nothing to decrypt the configuration with. D-06e must say that instead of implying recovery always continues. At **Public** there is no password at all, so the question never arises.

**The D-04/D-06 readout is TWO-STATE (decision 97, REC-25):** the level the user picked at setup decides what a fresh device can show **before** any password is entered. At **Private** — the default — **nothing** renders first: the entry is a locked state whose copy is **"we cannot show your setup yet — enter the recovery password from your card"**. At **Shape visible** and at **Public** the **structure** — THE path, its required rows and its group thresholds — renders straight from the **setup-event data, with no password at all**; at Shape visible the **values** (which guardians, which passkey, which document) stay masked until the card password unlocks them, and at Public they are already in the clear. Two consequences for the screens: the locked state must never be drawn as an empty path or a "no recovery set up" result — the account IS configured, we simply cannot read it yet; and the masked-values state must look like a *deliberate* mask (16 dots + `hidden` chip, decision 69), never like a load failure. Both states keep decision 90's honesty: a lost card plus a forgotten password at Private or Shape visible is a **blocker**, not a degrade.
...
flowchart TD
    ...
    C1 --> C
    C -->|Found, no recovery set up| X["'This account cannot be recovered.'<br/>Try another address. Fires ONLY when no<br/>setup commitment exists — a configured<br/>but unreadable account routes to the<br/>locked readout instead (decision 99)"]
    X --> C
    C -->|Found, a recovery is already running| AR["Band (REC-26): 'A recovery is already<br/>running for this account.'<br/>At most ONE attempt per account AT A TIME<br/>(their I-10) — show who/when if readable,<br/>the countdown, and the only two exits:<br/>wait it out, or cancel it (owner / counterproof)"]
    C -->|Found| CC["Confirm identity: address + blockie +<br/>ENS. 'This is my account'"]
    CC -->|Not mine| C
    CC --> V{"D-04 / D-06 readout — TWO-STATE<br/>by privacy level (decisions 84 + 97 + 99)"}
    V -->|"Private (default)"| VE["LOCKED state — nothing renders:<br/>'We cannot show your setup yet —<br/>enter the recovery password<br/>from your Recovery Card.'"]
    V -->|Shape visible or Public| VD["STRUCTURE renders with NO password:<br/>THE path (decision 73), read from the<br/>setup-event data. At Shape visible the<br/>VALUES — which guardians, which passkey,<br/>the thresholds — stay masked<br/>(16 dots + hidden chip, decision 69)"]
    VE --> VP["Enter the recovery password →<br/>values revealed. Wrong/absent password<br/>is a REAL blocker at Private and at<br/>Shape visible (decision 90), not a degrade"]
    VD --> VP
...
flowchart TD
    ...
    I -->|Submit REJECTED: attempt exists| IR["Distinct cause (REC-26): an attempt is<br/>already open and at most ONE can be open<br/>at a time (their I-10). Not a retry — wait<br/>it out or cancel that attempt; a new one<br/>can open once none is active"]
...
Reconciliation states (decision 96):
...
- ✅ Claims **survive pauses and resumes** on this device, **but they may expire** (decision 91). A resumed recovery **re-checks every stored claim's validity** before showing progress, and surfaces the expired ones instead of counting them as done. (Path switching no longer exists — single-path model, decision 73.)
...
Decisions:
- ✅ Banner + notification. Polling caveat accepted for now (user must open the browser during the waiting period). Alerts opt-in in Flow C step 9 mitigates.
...
**E1 (sync) is the default for v1.** Async on-chain approvals are skipped — decision 89; they would need a big recovery-contract redesign, and the alternatives are logged with that decision. The recovery **trigger stays with one person** (the recoverer).
...
```mermaid
flowchart TD
    A["Recoverer shares link/QR<br/>with the guardian (any channel)"] --> B["Guardian opens the approval page<br/>(extension-served path — decision 51;<br/>IPFS/IPNS later)"]
    ...
    G --> H["Recoverer pastes it — validated on paste,<br/>five errors: malformed · wrong recovery ·<br/>wrong signer · already added · expired.<br/>Each one surfaces immediately"]
```
...
- **A guardian approval DOES expire (decision 91 — this bullet said the opposite and was wrong).** `valid_until` is one of the eleven digest fields, and their invariant **I-13 rejects any stale proof at the door** — a submission carrying an expired approval fails. So an approval dies in **three** ways: its **expiry** passes, its **nonce is consumed**, or the request is **cancelled**. The page **shows** the expiry, as a plain deadline plus a countdown, because it is a real limit and a guardian who signs near the edge should know the recoverer has to submit before it lapses.
...
- Same machinery as the verify-access tests, run over time — an **optional capability, not the v1 focus** (decision 42); nothing else in this document depends on it shipping.
- **V2.** Decisions 42, 71 and 83 carry the detail: the passkey check is a user-gesture test, the "Last check" stat renders only when the capability is enabled, and the graded meter waits for the fragility grading of decision 86.
- Screens **F-01 / F-02** stay drawn, and the meter label stays **"Recovery ready"**, not "Protected" — recovery protects against key **loss**, not key theft.
...
flowchart TD
    ...
    D --> D2["Always the explicit warning<br/>(it is the only path): account becomes<br/>UNPROTECTED. Status drops to 'Not set up' (86).<br/>Type-to-confirm or equivalent.<br/>When a recovery is pending, add:<br/>'Removing recovery cancels the recovery<br/>currently in progress. Uninstalling the<br/>module does the same.' (REC-6)"]
...
- **G-02 removal warning (REC-6, MVP):** removing recovery uninstalls the module, so it carries the consequence line **"Removing recovery cancels the recovery currently in progress. Uninstalling the module does the same."** on top of the existing "your account becomes UNPROTECTED" warning.
...
- **Reconciliation consequences (decision 96):** the MVP grows **copy, not screens**, almost everywhere — eight of the eleven MVP items above are lines on screens that already exist (REC-17, REC-27, REC-30, REC-33, REC-13, REC-6 twice). The three that need real state are the **already-running band + rejected cause** (REC-26), the **expired-claim row** (REC-28), and the **fresh-device discovery help** (REC-31). One MVP honesty position gets **harder**, not softer: a lost card **plus** a lost password means no recovery at Private or Shape visible (decision 90).
- **Final ruling batch consequences (decisions 97–98):** the MVP again grows **copy plus one state**, not a screen family. The one real state is the **Private locked readout** on D-04/D-06 — an entry that says the account IS configured and we cannot read it yet, which is neither the masked-values state nor the "no recovery set up" error. Everything else is lines on screens that already exist: three reveal lines on the C-06e radio (#5), one sentence on each ID enrollment row and its C-07 echo (#4), and a **tag** on the decision-90 line that already states the backup trade (#6). The V1 side adds **no screens either** — #1/#2/#3 are the copy layer of the fragility grading that decision 86 already scheduled, so they ship when the grading rules exist and not before. One dependency to keep visible: **Shape visible** rests on decision 97's assumption that the kit supports the middle level at all (kit-ask REC-24, SDK assumption A7). With **Private** as the default (decision 99) that assumption is no longer load-bearing: a negative answer removes the middle option from the dial, and the default readout — the locked state — stays exactly as drawn.
...
- **MVP blockers (kit team):** **zkPassport integration + prover** (decision 80) · group encoding (T10) · confirm the execute-repays-`msg.sender` mechanic (87 — flagged as may-change). Closed since decision 79: value visibility (answered by 84, commit-reveal), the contract-enforced 24h floor (removed by 85 — the 24-hour minimum returns in decision 100 as a wallet-side picker floor, which blocks nothing), and funding-as-blocker (87: MVP is user-funded via the gas-deposit screens; paymaster sponsorship is V1)

## Decisions log

98+ rows. Superseded rows keep their amendment notes — this table is the history; the body above states only what is true now.
...
| 23 | Guardian gas (E2) | Needs sponsorship; one of E2's three blockers. **(E2 removed from the doc; decision 89.)** |
...
| 58 | Wizard step map | Ten numbered steps: 1 how-it-works · 2 inventory · 3 recommended path (singular since #73) · 4 enroll · 5 waiting period · 6 recovery password · 7 dry run · 8 review+save · 9 card+alerts · 10 apply-to-others. Single-account wallets skip 10 → "OF 9". Preset deep-links get their own landing (C-04d) with a renumbered short run. The fast-track onboarding is a prologue labeled "SET UP THIS DEVICE · N OF 4", so only one recovery counter exists. |
...
| 74 | Waiting period v2 | One waiting period per account (the path's). **Contract enforces a 24h minimum** — the picker floor is 24h. 48h stays the UI default. Dominated-path warning retired. Supersedes #12's guardrails. **Superseded by #85**: the contract minimum is removed; 0h is legal with a picker-only warning. **Note (#100):** the 24h minimum returns as a **UI floor** in the picker — still not a contract floor. |
...
| 96 | **Reconciliation states batch** (2026-08-25) | New states and copy from the contracts-spec reconciliation, with milestones. **MVP:** recovery-already-running band + distinct submit-rejected cause (REC-26) · fresh-device account discovery (REC-31) · per-claim expiry countdown, expired-claim state, "ask again" (REC-28). **MVP copy:** "your collected approvals are void" (REC-27) · counterproof cancel (REC-30) · helper-publication disclosure (REC-17) · destination key + "what this removes" (REC-33). **V1:** extra-door enumeration + the terminal that does not claim the account is clean (REC-34) · repayment row (REC-36) · guided re-setup follow-on (REC-39) · migrate-assets state (REC-40) · the counterproof cancel-gathering flow (REC-30). **V2:** post-recovery re-salting on the D-15 cleanup list (REC-37). **Amended by #101:** the "cancelling may not land" copy (REC-29) is retired and dropped from this batch. |
| 97 | **Privacy levels assumed available** (2026-08-25) | We **assume the kit supports all three hiding levels of #84**, including **"Hide the details" — structure in the clear, values encrypted**. Their current model (`encrypt(password_key, config)` · `config` · `empty`) has **no counterpart** for that middle shape. The level data rides the **contract’s event logs** (the setup event’s backup field — storage keeps only the commitment, their I-16); for "Hide the details" the log carries the structure in the clear and the values encrypted (Fibo, 2026-08-25). A stated assumption, not a confirmed fact: **kit-ask REC-24** carries the request to confirm it, and the SDK doc records it as assumption **A7**. The **default stays "Hide the details"** — we do not drop the middle level or re-scope the dial to hidden-vs-public. This **un-blocks the two-state D-04/D-06 readout (REC-25, now resolved)**: at **"Hide the details"** and **"Public"** the structure — THE path — renders from the setup-event data with **no password at all**, and only the VALUES wait for the card password; at **"Hide everything"** **nothing** renders before the password. From REC-1. **Renamed by #99:** "Hide everything" is now **Private** and becomes the **default**, "Hide the details" is now **Shape visible**; the locked readout is therefore the default branch. |
| 98 | **Setup disclosures adopted** (2026-08-25) | Ruling on the contracts spec's **Q-8** disclosure list. **MVP copy:** **#4 identity ≠ presence** — an ID proof shows what the document says, not that the person acted just now (C-05 zkPassport and Anon Aadhaar enrollment rows, repeated on the C-07 review) · **#5 what your privacy level reveals** — one reveal line per option on the C-06e visibility radio · **#6 the backup trade** — lose the card **AND** the password at a hidden level and a fresh device cannot recover (C-06e password step; this is the line #90 already put there, tagged — not a second line). **V1, with the grading work:** **#1 single point of failure** · **#2 shared failure** · **#3 adopt-at-your-own-risk** (a module with no trust-root declaration) — parts of #1/#2 exist on C-07 today and stay as they are; the full **graded** treatment ships with fragility grading (#86). **#7 extra doors** is already V1 via REC-34. The **eighth** disclosure (destination key on approval) is already applied (REC-33, #96). **The rendering guarantee is the UX's, never the SDK's** — REC-18 confirmed, reaffirming #81(a) and SDK assumption **A6**: the SDK supplies facts (trust-root declarations, tiers, method metadata) as data; we render them and we judge them. From REC-35 / REC-18. **Renamed by #99:** disclosure #5's per-level lines now read Private (default) / Shape visible / Public, and no line may claim a level hides the existence of a setup. |
| 99 | **Privacy levels renamed; Private is the default** (2026-08-27) | Display names: **Private** (the **default** — shape AND values encrypted) · **Shape visible** (shape in the clear, values encrypted) · **Public** (nothing encrypted, no recovery password). Mapping to the kit's modes: Private = encrypt-everything, Shape visible = the middle shape (kit-ask REC-24), Public = cleartext. Their **I-16** says "private by default" — this aligns us. **Existence is NEVER hidden** (their design.md: what no dial hides is that a setup exists at all), so copy must say "nobody can **READ** your setup", never "nobody can see that you have one". Amends #84 / #90 / #97 / #98. The SDK's **enum literals do NOT change** (Fibo) — display names only. |
| 100 | **24h wallet-side minimum waiting period** (2026-08-27) | The **Instant (0h)** option is removed from the picker; the minimum is **24 hours** and the default stays **48h**. **The chain enforces no floor** (their I-7) — this is the **wallet's own** floor, and copy must never claim the contract enforces it. Amends #85: the no-floor fact stands, the picker changes. |
| 101 | **The "cancelling may not land" note is removed** (2026-08-27) | Too edge-case for the surface (titi + Fibo). The spec drops the copy and the **D2-01 second banner state rolls back**. **REC-29 retired.** |
| 102 | **Single-method paths stay; the demo keeps Sam** (2026-08-27) | 1-1 remains a supported capability — their spec calls passkey-only a first-class supported shape — but **no preset is 1-1**. Sam's demo journey stays single-method and shows the qualified "one method" state. Reaffirms #36 / #49 / #90 against the review question. |
...
| R5 conflict (atomic vs incremental); E2 needs an approval function + indexing — **E2 removed from the doc (decision 89)** | Flow E2 (skipped) | 89 |
| Setup batching: write T7's one-UserOp answer into the spec | Wizard step 8 (review + save) | Q28 |
| ~~**Concurrency**: can one account hold two pending recoveries?~~ **ANSWERED (their I-10, via REC-26)**: **no — at most one attempt per account AT A TIME; a new attempt can open once none is active.** The warning band and the distinct submit-rejected cause ship in the MVP (decision 96); the attempt id is client-predicted, so an intervening attempt voids a whole gathering (REC-27) | Flow D chapters 1/3, Flow D2 | M10/I-10 |
