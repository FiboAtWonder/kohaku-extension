...
4. **Install dependencies**: Run `yarn setup` (never `yarn install` directly — `setup` also runs `allow-scripts` and `patch-package`). Then install the submodule deps with `cd src/ambire-common && npm install && cd ../..`.
